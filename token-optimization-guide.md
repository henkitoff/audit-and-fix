...
|-----------|-------------|----------|---------|
| Full (1st) | 74 dims × all files | Same (no map yet) | 0% |
| Incremental (2nd+) | 74 dims × all files | Changed files: all dims. Hot-spots: known dims. Clean: skip (+ 10% random) | **60-80%** |
| Targeted | 74 dims × selected files | Selected files: map-recommended dims first | **30-50%** |
...
```
Changed files (20) × all dims: 20 × 5K = large-scale tokens
...
```
Full audit:    74 dimensions x ~5K tokens/agent = ~370K tokens
