---
name: audit-and-fix
description: Use when starting a comprehensive codebase audit, after major feature additions, before releases, or when multiple bugs suggest systemic issues — explores 73 dimensions across 7 rounds then fixes in parallel phases with Opus reviews
---
...
Multi-round, multi-agent codebase audit across 73 dimensions in 7 rounds, followed by parallel fix phases with Opus reviews and /simplify after each phase.
...
4. **Full audit:** When ready, run `/audit-and-fix --preset full` for all 73 dimensions.
...
| 7 | Token & API Cost Efficiency (prompt size, model routing, caching, ...) | 10 |
...
**Total:** ~20-30 agents, ~6-8 hours, 73 dimensions, 3+ Opus reviews, 3+ /simplify runs

## Presets

- `--preset quick`: Round 1 (Code) + Round 4 (Architecture) only. ~2 hours.
- `--preset full`: All 73 dimensions, all 7 rounds. ~6-8 hours.
...
| File | Purpose |
|------|---------|
| `exploration-dimensions.md` | Index of all 73 dimensions (links to round files) |
